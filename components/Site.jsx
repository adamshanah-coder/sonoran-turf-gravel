  ...
  return (
    <header className={`siteHeader ${scrolled ? 'isScrolled' : ''}`}>
      ...
      <nav className={open ? 'open' : ''} aria-label="Main navigation">
        ...
        <a href="#financing" onClick={close}>Financing</a>
...
function Studio() {
  ...
  return (
    <section id="studio" className="studio sectionPad">
      <div className="studioImage"><img src="/project-travertine.png" alt="Modern Arizona backyard design concept" /><span>Concept visualization</span></div>
      <div className="studioCopy">
        ...
        <label className="uploadButton">
          ...
          {fileName || 'Choose a yard photo'} <Arrow />
        </label>
...
function Financing() {
  ...
  const monthly = Math.round(amount / 84);
  return (
    <section id="financing" className="financing sectionPad">
      <div>
        <p className="kicker">Flexible financing</p>
        <h2>Build the complete vision.</h2>
        <p>Explore a comfortable starting point for your outdoor investment. Final terms, rates, and approval are provided by the financing partner.</p>
        <a className="textLink" href="#contact">Ask about financing <Arrow /></a>
      </div>
      <div className="financeCard">
        <div className="financeTop"><span>Project investment</span><strong>${amount.toLocaleString()}</strong></div>
        <input aria-label="Project investment" type="range" min="10000" max="150000" step="5000" value={amount} onChange={event => setAmount(Number(event.target.value))} />
        <div className="rangeLabels"><span>$10k</span><span>$150k</span></div>
        <div className="estimate"><span>Illustrative payment from</span><strong>${monthly.toLocaleString()}<small>/mo</small></strong></div>
        <p>Example uses a simple 84-month division and is not a financing offer or quoted payment.</p>
      </div>
    </section>
  );
}
...
export default function Site() {
  return <main><Header /><Hero /><Intro /><Services /><Projects /><Process /><Studio /><Financing /><About /><Areas /><Contact /><Footer /></main>;
}
